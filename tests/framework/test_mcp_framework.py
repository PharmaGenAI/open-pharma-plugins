...
def test_system_report_marks_missing_and_collapses_dormant(monkeypatch):
    ...
    deps = [
        ...
        {"label": "dormant-tool", "extra": "optional-dep", "probe": "__absent__", "tools": ["z"], "hint": "hintC"},
    ]
    ...
    assert "extras not installed" in rep and "optional-dep" in rep
...
def test_config_report_is_provider_neutral(tmp_path, monkeypatch):
    monkeypatch.setenv("OPEN_PHARMA_CONFIG_DIR", str(tmp_path))
    report = fw.config_report("open-pharma-plugins-hcp-intelligence")
    assert "capability settings available" in report
    assert "DASHSCOPE" not in report
