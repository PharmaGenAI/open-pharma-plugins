...
REPO_WEB_URL = "https://github.com/PharmaGenAI/open-pharma-plugins"
SEMVER = re.compile(
    r"(?P<major>0|[1-9]\d*)\."
    r"(?P<minor>0|[1-9]\d*)\."
    r"(?P<patch>0|[1-9]\d*)"
...
def verify_release_metadata(repo: Path, target: str, cap: str) -> tuple[str, str]:
    ...
    entry = next(
        (item for item in marketplace.get("plugins", []) if item.get("name") == f"open-pharma-plugins-{cap}"),
        None,
    )
    if entry is None:
        raise SystemExit(f"marketplace at {target} does not list open-pharma-plugins-{cap}")
    source = entry.get("source")
    if not isinstance(source, dict) or source.get("ref") != tag:
        raise SystemExit(f"marketplace at {target} does not pin {cap} to {tag}")

    copilot_marketplace = read_json_at(repo, target, ".github/plugin/marketplace.json")
    copilot_entry = next(
        (item for item in copilot_marketplace.get("plugins", []) if item.get("name") == f"open-pharma-plugins-{cap}"),
        None,
    )
    if copilot_entry is None:
        raise SystemExit(f"Copilot marketplace at {target} does not list open-pharma-plugins-{cap}")
    copilot_source = copilot_entry.get("source")
    if not isinstance(copilot_source, dict) or copilot_source.get("ref") != tag:
        raise SystemExit(f"Copilot marketplace at {target} does not pin {cap} to {tag}")
    if copilot_entry.get("version") != version:
        raise SystemExit(f"Copilot marketplace at {target} is not version {version} for {cap}")
...
def previous_tag(repo: Path, target: str, cap: str, version: str) -> str | None:
    prefix = f"open-pharma-plugins-{cap}-v"
...
def render_notes(
    ...
    lines = [f"open-pharma-plugins-{cap} {version}", ""]
    if previous is None:
