...
def update_distribution_version(index: dict, marketplaces: tuple[dict, ...], version: str) -> None:
...
def update_manifests(cap: str, version: str, tag: str) -> None:
    ...
    for rel in (
        ...
        if rel == ".claude-plugin/plugin.json" and "mcpServers" in data:
            ...
            server["args"][from_index] = f"open-pharma-plugins[{cap}] @ git+{REPO_URL}@{tag}"
        dump(path, data)
...
def main() -> int:
    ...
    entry = next((p for p in market["plugins"] if p["name"] == f"open-pharma-plugins-{cap}"), None)
    if entry is None:
        raise SystemExit(f"marketplace: missing open-pharma-plugins-{cap}")
    copilot_market = load(COPILOT_MARKETPLACE)
    copilot_entry = next(
        (p for p in copilot_market["plugins"] if p["name"] == f"open-pharma-plugins-{cap}"),
        None,
    )
    if copilot_entry is None:
        raise SystemExit(f"Copilot marketplace: missing open-pharma-plugins-{cap}")
    versions[cap] = version
    update_distribution_version(index, (market, copilot_market), args.distribution_version)
    entry["source"] = {
        "source": "git-subdir",
        "url": REPO_URL,
    ...
    dump(COPILOT_MARKETPLACE, copilot_market)

    print(f"Prepared open-pharma-plugins-{cap} {version}")
