...
(capability name / plugin name / MCP-server key / console entry are all `open-pharma-plugins-<cap>`)
...
- .claude-plugin/marketplace.json and .github/plugin/marketplace.json — each published plugin uses
  its harness-native source shape pinned to the canonical immutable tag; both marketplace metadata
  versions match mcp_framework.
- src/capabilities/<cap>/{.claude-plugin,.codex-plugin}/plugin.json (+ .mcp.json) —
...
- pyproject.toml — [project.scripts] has `open-pharma-plugins-<cap> = "<import_name>.__main__:main"`
...
def release_index() -> tuple[str | None, dict[str, str], str]:
    ...
    if not isinstance(data, dict):
        return None, {}, "open-pharma-plugins-{cap}-v{version}"
...
def check_launch_ref(server_cfg: dict, cap: str, tag: str, where: str) -> None:
    ...
    try:
        ...
        return
    expected_extra = f"open-pharma-plugins[{cap}]"
...
def check_capability(
    ...
    if import_name is not None:
        ...
        if not mcp_json_path.is_file():
            fail(f"{rel(cap_dir)}: server capability without .mcp.json (codex manifest needs it)")
        else:
...
def main() -> int:
    ...
    for name, target in AUXILIARY_SCRIPTS.items():
        if scripts.get(name) != target:
            fail(f"pyproject.toml [project.scripts]: {name} = {scripts.get(name)!r}, expected {target!r}")
    ...
    # Copilot marketplace ↔ release index and Claude marketplace. Copilot supports the existing
    # per-capability .claude-plugin/plugin.json, but its marketplace requires a `github` source
    # object rather than Claude's `git-subdir` object.
    copilot_marketplace = load_json(COPILOT_MARKETPLACE)
    copilot_listed: set[str] = set()
    if copilot_marketplace is not None:
        meta_version = copilot_marketplace.get("metadata", {}).get("version")
        if distribution_version is not None and meta_version != distribution_version:
            fail(
                f"{rel(COPILOT_MARKETPLACE)}: metadata.version {meta_version!r} != "
                f"release index distribution_version {distribution_version!r}"
            )
        if copilot_marketplace.get("name") != "open-pharma-plugins":
            fail(f"{rel(COPILOT_MARKETPLACE)}: marketplace name must be 'open-pharma-plugins'")
        names = [p.get("name") for p in copilot_marketplace.get("plugins", [])]
        for name in {n for n in names if names.count(n) > 1}:
            fail(f"{rel(COPILOT_MARKETPLACE)}: duplicate plugin name {name!r}")
        for plugin in copilot_marketplace.get("plugins", []):
            name, source = plugin.get("name"), plugin.get("source")
            if not isinstance(name, str) or not name.startswith(PREFIX):
                fail(f"{rel(COPILOT_MARKETPLACE)}: invalid plugin name {name!r}")
                continue
            cap = name.removeprefix(PREFIX)
            copilot_listed.add(cap)
            version = versions.get(cap)
            expected_source = {
                "source": "github",
                "repo": "PharmaGenAI/open-pharma-plugins",
                "path": f"src/capabilities/{cap}",
                "ref": tag_format.format(cap=cap, version=version) if version is not None else None,
            }
            if source != expected_source:
                fail(f"{rel(COPILOT_MARKETPLACE)}: {name}: source {source!r}, expected {expected_source!r}")
            if plugin.get("version") != version:
                fail(
                    f"{rel(COPILOT_MARKETPLACE)}: {name}: version {plugin.get('version')!r} "
                    f"!= release index {version!r}"
                )
            cap_dir = CAPABILITIES_DIR / cap
            if not (cap_dir / ".claude-plugin" / "plugin.json").is_file():
                fail(f"{rel(COPILOT_MARKETPLACE)}: {name}: source has no reusable .claude-plugin/plugin.json")

    if copilot_listed != listed:
        fail(
            "Claude and Copilot marketplaces must list the same capabilities: "
            f"claude={sorted(listed)}, copilot={sorted(copilot_listed)}"
        )

    # Reverse direction: nothing stale in pyproject.
    cap_names = {d.name for d in caps}
    for entry in scripts:
        if entry in AUXILIARY_SCRIPTS:
            continue
    ...
    for import_name, target in package_dir.items():
        if import_name.startswith("open_pharma_plugins") and not (REPO_ROOT / target).is_dir():
    ...
    if errors:
        ...
        return 1
    print(
        f"OK — {len(caps)} capabilities consistent across Claude/Copilot marketplaces, "
        "plugin manifests and pyproject.toml"
    )
    return 0
