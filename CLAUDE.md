...
- A capability lives under `src/capabilities/<cap>/` and may ship a Skill, an MCP server, or both.
  `hcp-intelligence` profiles HCPs/HCOs from public data.
  `field-training` generates learning packages, assessments, and role-play from approved documents.
- Keep names aligned: folder `<cap>`, Skill/plugin/entry `open-pharma-plugins-<cap>`, extra `[<cap>]`,
  and import `open_pharma_plugins_<cap-with-underscores>`.
...
- All MCP servers ship in one `open-pharma-plugins` distribution. Extras select dependencies, not
...
Do not duplicate registration or schemas manually. Every manifest server key must use the unique
capability name.
